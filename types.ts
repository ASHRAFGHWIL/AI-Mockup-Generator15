...
export type DesignStyle = 'classic' | 'split' | 'sketch' | 'slasher' | 'vintage_stamp' | 'retro_wave' | 'minimalist_line' | 'grunge_overlay' | 'full_wrap' | 'full_front' | 'cyberpunk_glitch' | 'stacked_text' | 'emblem' | 'photo_text' | 'american_traditional_tattoo' | 'watercolor_splash' | 'art_deco' | 'pop_art' | 'cosmic_galaxy' | 'japanese_ukiyo-e' | 'distressed_vintage' | 'typography_focus' | 'abstract_geometric' | 'vintage_poster' | 'geometric_pattern' | 'hand_drawn_sketch' | 'boho_floral_wreath' | 'girly_script_heart' | 'celestial_moon_phases' | 'glam_leopard_print' | 'kawaii_cute_doodle' | 'watercolor_blooms' | 'gold_foil_accents' | 'lace_trim_border' | 'pastel_tie_dye' | 'starry_night_sky' | 'cherry_blossom_dream' | 'satin_lace_trim' | 'rose_gold_glitter' | 'dreamy_angel_wings' | 'mystical_smoke_perfume' | 'full_circle' | 'upper_half_circle' | 'lower_half_circle' | 'upper_oval' | 'lower_oval';
export type ModelPose = 'flat_lay_simple' | 'standing' | 'sitting' | 'sitting_floor_cozy' | 'recumbent' | 'smiling_glasses' | 'back' | 'drinking_tea' | 'jumping' | 'dancing' | 'meditating' | 'heroic' | 'action' | 'yoga' | 'casual_lean' | 'walking_street' | 'laughing' | 'arms_crossed' | 'thinking' | 'hands_in_pockets' | 'closeup_casual' | 'sitting_hand_hip' | 'sitting_on_counter' | 'elegant_shoulder_look' | 'gentle_touch_face' | 'windswept_hair_dreamy' | 'lounging_on_silk' | 'playful_peekaboo' | 'cozy_sweater_hug' | 'sun_kissed_glow_portrait' | 'playful_hair_toss_laugh' | 'meadow_daydream_top_down' | 'city_lights_glamour' | 'delicate_flower_touch' | 'parisian_balcony_daydream' | 'cozy_cafe_window_rain' | 'secret_garden_reading' | 'vintage_car_glamour' | 'sitting_floor_relaxed' | 'sitting_cross_legged_side' | 'leaning_on_floor' | 'standing_three_quarter_hands_pockets' | 'standing_rear_three_quarter' | 'sitting_top_down' | 'low_angle_arms_crossed' | 'closeup_chest_design' | 'full_body_slouched' | 'sitting_on_chair_dynamic';
...
export type PillowSetting = 'on_sofa' | 'on_bed' | 'on_armchair' | 'luxury_bedroom_setting' | 'plush_window_nook' | 'chic_velvet_sofa' | 'faux_fur_rug_romance' | 'dreamy_chaise_lounge';
// Flat Lay Types
